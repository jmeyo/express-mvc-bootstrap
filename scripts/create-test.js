...
exports.execute = function(params, appPath) {
  ...
  if (params.length == 0) {
    console
        .log('You must specifiy a model name to generate the tests against!');
    return;
  }
  ...
  var integrationTemplate = __dirname +
                    '/templates/create-test.template.integration.ejs';
  var functionalTemplate = __dirname +
                    '/templates/create-test.template.functional.ejs';
  ...
  if (fileCheck) {
    if (params[1] != 'force') {
      ...
      console.log('Add an additional paramater of "force" ' +
                  ' to over write the tests.');
  ...
  fs.writeFileSync(testFolder + '/integration/' + controllerName +
                    'Controller.js', retIntegration, 'utf8');
  fs.writeFileSync(testFolder + '/functional/' + modelName + '.js',
      retFunctional, 'utf8');
