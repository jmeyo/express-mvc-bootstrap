...
exports.execute = function(params, appPath) {
  ...
  if (params.length == 0) {
    console
        .log('You must specifiy a model name to generate the ' +
            ' controller against!');
    return;
  }
  ...
  var controllerFile = appPath + '/controllers/' + controllerName +
                       'Controller.js';

  var controllerTemplate = __dirname +
                           '/templates/create-controller.template.ejs';
  ...
  if (fileCheck) {
    if (params[1] != 'force') {
      ...
      console
          .log('Add an additional paramater of "force" to over ' +
  ...
  console.log('Controller for model ' + modelName +
              ' created in controllers/' +
