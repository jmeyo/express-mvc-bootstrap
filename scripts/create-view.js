...
exports.execute = function(params, appPath) {
  ...
  if (params.length == 0) {
    console
        .log('You must specifiy a model name to generate the views against!');
    return;
  }
  ...
  var viewIndexTemplate = __dirname +
                    '/templates/create-view.template.index.ejs';
  ...
  // Render the views
  var retIndex = ejs.render(tmpIndex, {
    locals: {
      modelName: modelName,
  ...
  var retEdit = ejs.render(tmpEdit, {
    locals: {
      modelName: modelName,
      controllerName: controllerName
    },
  ...
  var retShow = ejs.render(tmpShow, {
    locals: {
      modelName: modelName,
      controllerName: controllerName
    },
  ...
  console.log('Views ' + modelName + ' created in views/' +
